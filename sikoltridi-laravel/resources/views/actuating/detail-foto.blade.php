...
<div class="bg-gray-50 min-h-screen pt-24 py-10 px-4 sm:px-6 lg:px-8 pb-28">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <img src="{{ asset('uploads/foto/' . $foto->image_file) }}" 
                 alt="{{ $foto->title }}"
                 class="w-full object-contain max-h-[70vh] bg-gray-100">
        </div>

        <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 mb-2">{{ $foto->title }}</h1>
            <div class="flex items-center text-gray-500 text-sm mb-3">
                <span>Diupload pada {{ \Carbon\Carbon::parse($foto->uploaded_at)->translatedFormat('d F Y') }}</span>
            </div>
            <p class="text-gray-700 leading-relaxed">{{ $foto->deskripsi_image ?? 'Tidak ada deskripsi.' }}</p>
        </div>

        <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800 mb-4">Komentar ({{ $comments->count() }})</h2>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-300 shadow-lg py-3 px-4">
        <form method="POST" action="{{ route('actuating.foto.comment') }}" class="max-w-4xl mx-auto flex items-center space-x-3">
            @csrf
