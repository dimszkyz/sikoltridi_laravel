<div class="p-6">
    <div class="bg-white shadow-md rounded-lg p-6">
        <div class="text-center mb-8">
            <h2 class="text-3xl font-bold text-gray-800">File</h2>
        ...
        <div class="flex flex-wrap gap-6 justify-center">
            @forelse($files as $file)
                <div class="relative overflow-hidden rounded-md shadow-md group cursor-pointer w-[300px]"
                     onclick="window.open('{{ asset('uploads/files/' . $file->pdf_file) }}', '_blank')">
                    
                    <img
                        src="{{ $file->image_file ? asset('uploads/images/' . $file->image_file) : 'https://via.placeholder.com/100x150?text=No+Preview' }}"
                        alt="{{ $file->title }}"
